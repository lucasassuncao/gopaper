// Package updater implements the self-update mechanism for gopaper.
package updater
...
// SelfUpdate downloads a release of gopaper from GitHub and replaces
// the current binary. The old binary is kept as <name>.old until the next run,
// when it is cleaned up automatically.
//
// repo must be in "owner/repo" format, e.g. "lucasassuncao/gopaper".
...
func ListReleases(repo, token string, includePrerelease bool, limit int) ([]Release, error) {
	if repo == "" {
		return nil, fmt.Errorf("repo is required (e.g. lucasassuncao/gopaper)")
	}
	if limit <= 0 {
		limit = 20
	}
...
func newGitHubRequest(method, rawURL, token string) (*http.Request, error) {
	...
	req.Header.Set("User-Agent", "github.com/lucasassuncao/gopaper")
	if token != "" {
...
// checked against the lowercased asset name.
var checksumManifestPatterns = []string{"checksum", "sha256sums", ".sha256"}

// verifyDownloadedAsset checks the downloaded binary at downloadedPath against
// a checksum manifest published alongside asset in the release, if one exists.
// A missing manifest is not an error — GitHub releases don't always publish
// one — but a mismatch against a manifest that does exist is.
